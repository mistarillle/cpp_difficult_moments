...
std::ostream& operator<<(std::ostream& out, const Person& person) {
    ...
    if (person.m_patronymic and person.m_patronymic.value() != "-") {
...
bool operator<(const Person& person1, const Person& person2) {
    //Comparing sum of letters in persona's name

    std::size_t person1_length = person1.m_last_name.length() + person1.m_first_name.length();
    std::size_t person2_length = person2.m_last_name.length() + person2.m_first_name.length();
    if (person1.m_patronymic) person1_length += person1.m_patronymic->length();
    if (person2.m_patronymic) person2_length += person2.m_patronymic->length();
    return person1_length < person2_length;
...
struct PhoneNumber {
    ...
    std::optional<int> m_additional_number = 0;
...
std::ostream& operator<<(std::ostream& out, PhoneNumber& phone_number) {
    ...
    if (phone_number.m_additional_number and phone_number.m_additional_number.value() != 0) {
...
public:
    ...
    void SortByName() {
        ...
        std::sort(m_phone_book.begin(), m_phone_book.end(),
                  ...
                  {
                        if (pair1.first.m_patronymic.has_value() and pair2.first.m_patronymic.has_value()) {
                            if (pair1.first.m_first_name == pair2.first.m_first_name) {
                                return pair1.first.m_patronymic.value() < pair2.first.m_patronymic.value();
                            }
                            if (pair1.first.m_last_name == pair2.first.m_last_name) {
                                return pair1.first.m_first_name < pair2.first.m_last_name;
                            }
                            return pair1.first.m_last_name < pair2.first.m_last_name;
                        }
                  });
    }
    ...
    void SortByPhone() {
        ...
        std::sort(m_phone_book.begin(), m_phone_book.end(),
                  [](std::pair<Person,PhoneNumber>& pair1, std::pair<Person,PhoneNumber>& pair2)
                    {
                        if (pair1.second.m_additional_number.has_value() and pair2.second.m_additional_number.has_value())
                        {
    ...
    std::tuple<std::string, PhoneNumber> GetPhoneNumber(const std::string& last_name) {
        ...
        if (tmp_vector.empty()) {
            result = "not found";
            //Чтобы вернуть пустой объект PhoneNumber, пришлось создавать объект-заглушку.
            //Вернуть пустой объект можно как-то по другому?
